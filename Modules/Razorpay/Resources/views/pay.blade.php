...
<div class="container-fluid container-fluid-90 margin-top-85 min-height">
	...
	<div class="row">
		<div class="col-md-8 col-sm-8 col-xs-12 mb-5 main-panel p-5 border rounded">
			<div class="pb-3 m-0 text-24 font-weight-700">{{ __('RazorPay') }} {{ __('Payment') }} </div>
			<form action="{{ route('gateway.complete', ['gateway' => config('razorpay.alias')]) }}" method="post" id="payment-form">
				{{ csrf_field() }}
				
				<button id="rzp-button" type="submit" class="pay-button sub-btn btn vbtn-outline-success text-16 font-weight-700 pl-5 pr-5 pt-3 pb-3">{{ __('Pay with Razorpay') }}</button>
